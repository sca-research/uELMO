#include <stdint.h>
#include <stdbool.h>
#include "MaskedAesFull.h"

#define MAX_LINE_LEN ((256))

#define CMD_RESERVED ((0x00))
#define CMD_SET_KEY ((0x01))
...
static char ToAscii(uint8_t fourbits)
{
    const char *Digits = "0123456789ABCDEF";
    char c = Digits[0x0F & fourbits];

    return c;
}

static void PrintByteHex(uint8_t u)
{
    unsigned char cl, ch;
...
unsigned char xtime[256] = {
    ...
    123, 121, 127, 125, 115, 113, 119, 117, 107, 105, 111, 109, 99, 97, 103,
    101,
    155, 153, 159, 157, 147, 145, 151, 149, 139, 137, 143, 141, 131, 129, 135,
    133,
    187, 185, 191, 189, 179, 177, 183, 181, 171, 169, 175, 173, 163, 161, 167,
    165,
    219, 217, 223, 221, 211, 209, 215, 213, 203, 201, 207, 205, 195, 193, 199,
    197,
...
static void SetKey(uint8_t *key)
{
    ...
    PrintLine("Key updated\n", MAX_LINE_LEN);
    return;
}

static void Encrypt(uint8_t *key)
{
    uint8_t plain[16] = { 0 };
    uint8_t cipher[16] = { 0 };
...
int main(void)
{
    ...
    while (!exit)
    {
        ...
        default:               //Unspecified command code. Ignore.
            break;
        }
    }
