...
void write32 ( unsigned int addr, unsigned int data )
{
    ...
    switch(addr&0xF0000000)
    {
		...
		default: // Do two write 16s
            if(DEBUG_MEM) printf("write32(0x%08X,0x%08X)\n",addr,data);
...
unsigned int read16 ( unsigned int addr )
{
    ...
    switch(addr&0xF0000000)
    {
        ...
        default: //RAM
            ...
            if(DEBUG_MEM) printf("read16(0x%08X)=",addr);
            addr&=RAMADDMASK;
            addr>>=1;
            data=ram[addr];
...
unsigned int read32 ( unsigned int addr )
{
    ...
    switch(addr&0xF0000000)
    {
        ...
        default: //Do two read16s
            if(DEBUG_MEM) printf("read32(0x%08X)=",addr);
            data =read16(addr+0);
            data|=((unsigned int)read16(addr+2))<<16;//higher address->high bits
            if(DEBUG_MEM) printf("0x%08X\n",data);
            return(data);
    }
...
bool Memory_OneCycle()
{
	...
	if(core_current.Read_valid)//Performing read
	{
		...
		core_current.Memory_read_targetreg_buf=core_current.Memory_read_targetreg;
        ...
        sprintf(core_current.Memory_instr_disp, "Memory: load [0x%X]=0x%X", core_current.Memory_addr, value);
	...
	if(core_current.Write_valid)//Performing write
	{
        ...
        sprintf(core_current.Memory_instr_disp, "Memory: store [0x%X]=0x%X", core_current.Memory_addr, core_current.Memory_data);
