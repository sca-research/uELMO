...
bool Execute_OneCylce(bool wait_mem)
{
    ...
    if((inst & 0xF000) == 0xD000)
    {
        ...
        case 0xA:              //b ge N == V
            ra = 0;
            if((core_current.cpsr & CPSR_N) && (core_current.cpsr & CPSR_V))
                ra++;
            if((!(core_current.cpsr & CPSR_N))
        ...
        case 0xB:              //b lt N != V
            ...
            if((!(core_current.cpsr & CPSR_N)) && (core_current.cpsr & CPSR_V))
                ra++;
            if((!(core_current.cpsr & CPSR_V)) && (core_current.cpsr & CPSR_N))
                ra++;
            if(ra)
            {
                rc = rb;
    ...
    if((inst & 0xFE00) == 0xB400)
    {
        ...
        if(rb_ind == 9)
        {
            ...
            if(inst & 0x100)    //push lr
            {
                ...
                core_current.Memory_addr = ra;
                sprintf(core_current.Execute_instr_disp,
                        "Execute: push lr=0x%X", core_current.Memory_data);
        ...
        if(core_current.Execute_multicycle_regindex == 9)       //First cycle
        {
            ...
            if(DEBUG_CORE)
                printf("Execute C1:str r%u,[r%u,r%u]\n", rd_ind, ra_ind,
                       rb_ind);
            sprintf(core_current.Execute_instr_disp,
                    "Execute C1: str r%u,[r%u,r%u]", rd_ind, ra_ind, rb_ind);
        ...
        else                    //Second cycle
        {
            ...
            if(DEBUG_CORE)
                printf("Execute C2:str r%u,[r%u,r%u]\n", ra_ind, rd_ind,
                       rb_ind);
            sprintf(core_current.Execute_instr_disp,
                    "Execute C2: str r%u,[r%u,r%u]", ra_ind, rd_ind, rb_ind);
            rd_ind = 0xff;      //not used
            core_current.Execute_multicycle_regindex = 9;
    ...
    if((inst & 0xFE00) == 0x5400)
    {
        if(core_current.Execute_multicycle_regindex == 9)       //First cycle
        {
            ...
            sprintf(core_current.Execute_instr_disp,
                    "Execute C1:strb r%u,[r%u,r%u]", rd_ind, ra_ind, rb_ind);
        ...
        else                    //Second cycle
        {
            ...
            sprintf(core_current.Execute_instr_disp,
                    "Execute C2:strb r%u,[r%u,r%u]", ra_ind, rd_ind, rb_ind);
            rd_ind = 0xff;      //not used
            core_current.Execute_multicycle_regindex = 9;
            core_current.Memory_data = ra;      //sent to memory
            core_current.Write_valid = true;
    ...
    if((inst & 0xFE00) == 0x5200)
    {
        if(core_current.Execute_multicycle_regindex == 9)       //First cycle
        {
            ...
            sprintf(core_current.Execute_instr_disp,
                    "Execute C1:strh r%u,[r%u,r%u]", rd_ind, ra_ind, rb_ind);
            core_current.Execute_multicycle_regindex = 0;
            //Functional
            rc = ra + rb;
            //core_current.Memory_data=rb;//sent to memory
        ...
        else                    //Second cycle
        {
            ...
            sprintf(core_current.Execute_instr_disp,
                    "Execute C2:strh r%u,[r%u,r%u]", ra_ind, rd_ind, rb_ind);
