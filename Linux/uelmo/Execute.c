...
bool Execute_OneCylce(bool wait_mem)
{
    ...
    bool IsLDR = ((inst & 0xF800) == 0x6800) || ((inst & 0xFE00) == 0x5800)
	|| ((inst & 0xF800) == 0x4800) || ((inst & 0xF800) == 0x9800)
    ...
    if ((inst & 0xFFC0) == 0x4140)
	{
        ...
        update_component(&rb, &(core_current.D2E_reg2));
	    ...
	    sprintf(core_current.Execute_instr_disp, "Execute: adc r%u,r%u\n",
		    ra_ind, rb_ind);
	    //Logical calculation 
	    //rc.num_value = ra.num_value + rb.num_value;
        //TODO: update rc.exp
	    rc.num_value = ra.num_value + rb.num_value;
        //TODO: update rc.exp

        if (core_current.cpsr & CPSR_C)
		rc.num_value = rc.num_value + 1;

	    //Update CPSR
	    do_nflag(rc.num_value);
	    do_zflag(rc.num_value);
	    if (core_current.cpsr & CPSR_C)
		{
		    do_cflag(ra.num_value, rb.num_value, 1);
		    do_vflag(ra.num_value, rb.num_value, 1);
		}
	    else
		{
		    do_cflag(ra.num_value, rb.num_value, 0);
		    do_vflag(ra.num_value, rb.num_value, 0);
		}
    ...
    if ((inst & 0xF800) == 0x3000)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp,
		    "Execute: adds r%u,#0x%02X", rd_ind, rb.num_value);
    ...
    if ((inst & 0xFE00) == 0x1800)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp,
		    "Execute: adds r%u,r%u,r%u", rd_ind, ra_ind, rb_ind);
	    //Functional
	    rc.num_value = ra.num_value + rb.num_value;
        //TODO: update rc.exp
    ...
    if ((inst & 0xFF00) == 0x4400)
	{
	    ...
	    if (DEBUG_CORE)
		printf("add r%u,r%u\n", ra_ind, rb_ind);
	    sprintf(core_current.Execute_instr_disp, "Execute: add r%u,r%u",
		    ra_ind, rb_ind);
    ...
    if ((inst & 0xF800) == 0xA000)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp,
		    "Execute: add r%u,PC,#0x%02X", rd_ind, rb.num_value);
	    //Functional
	    rc.num_value = ra.num_value + rb.num_value;
        //TODO: update rc.exp
    ...
    if ((inst & 0xF800) == 0xA800)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp,
		    "Execute: add r%u,SP,#0x%02X", rd_ind, rb.num_value);
    ...
    if ((inst & 0xFF80) == 0xB000)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp, "Execute: add SP,#0x%02X",
		    rb.num_value);
    ...
    if ((inst & 0xFFC0) == 0x4000)
	{
	    ...
	    if (DEBUG_CORE)
		printf("ands r%u,r%u\n", ra_ind, rb_ind);
	    sprintf(core_current.Execute_instr_disp, "Execute: ands r%u,r%u",
		    ra_ind, rb_ind);
    ...
    if ((inst & 0xF800) == 0x1000)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp,
		    "Execute: asrs r%u,r%u,#0x%X", rd_ind, ra_ind, rb.num_value);

	    //Functional
	    rc.num_value = ra.num_value;
        //TODO: update rc.exp
	    if (rb.num_value == 0)
		{
		    if (rc.num_value & 0x80000000)
			{
			    rc.num_value = ~0;
			}
		    else
		...
		{
		    ra.num_value = rc.num_value & 0x80000000;
		    rc.num_value >>= rb.num_value;
            //TODO: update ra.exp rc.exp
		    if (ra.num_value)	//asr, sign is shifted in
			{
			    rc.num_value |= (~0) << (32 - rb.num_value);
                //TODO: update rc.exp
			}
		};
    ...
    if ((inst & 0xFFC0) == 0x4100)
	{
	    ...
	    if (DEBUG_CORE)
		printf("asrs r%u,r%u\n", ra_ind, rb_ind);
	    sprintf(core_current.Execute_instr_disp, "Execute: asrs r%u,r%u",
		    ra_ind, rb_ind);
	    ...
	    else if (rb.num_value < 32)
		{
		    ra.num_value = rc.num_value & 0x80000000;
		    rc.num_value >>= rb.num_value;
            //TODO: update ra.exp and rc.exp
		    if (ra.num_value)	//asr, sign is shifted in
			{
			    rc.num_value |= (~0) << (32 - rb.num_value);
                //TODO: update rc.exp
    ...
    if ((inst & 0xF000) == 0xD000)
	{
        ...
        read_register(15, &rc);
        rc.num_value = rc.num_value - 2;
        //TODO: update rc.exp
	    rb.num_value <<= 1;
	    rb.num_value = rb.num_value + rc.num_value;
	    rb.num_value = rb.num_value + 2;
        //TODO: update rb.exp
		...
		case 0x0:	//b eq  z set
			...
			printf("beq 0x%08X\n", rb.num_value - 3);
		    sprintf(core_current.Execute_instr_disp,
			    "Execute: beq 0x%08X", rb.num_value - 3);
		...
		case 0x1:	//b ne  z clear
			...
			printf("bne 0x%08X\n", rb.num_value - 3);
		    sprintf(core_current.Execute_instr_disp,
			    "Execute: bne 0x%08X", rb.num_value - 3);
		    break;
		...
		case 0x2:	//b cs c set
			...
			printf("bcs 0x%08X\n", rb.num_value - 3);
		    sprintf(core_current.Execute_instr_disp,
			    "Execute: bcs 0x%08X", rb.num_value - 3);
		    break;
		...
		case 0x3:	//b cc c clear
			...
			printf("bcc 0x%08X\n", rb.num_value - 3);
		    sprintf(core_current.Execute_instr_disp,
			    "Execute: bcc 0x%08X", rb.num_value - 3);
		    break;
		...
		case 0x4:	//b mi n set
			...
			printf("bmi 0x%08X\n", rb.num_value - 3);
		    sprintf(core_current.Execute_instr_disp,
			    "Execute: bmi 0x%08X", rb.num_value - 3);
		    break;
		...
		case 0x5:	//b pl n clear
			...
			printf("bpl 0x%08X\n", rb.num_value - 3);
		    sprintf(core_current.Execute_instr_disp,
			    "Execute: bpl 0x%08X", rb.num_value - 3);
		    break;
		...
		case 0x6:	//b vs v set
			...
			printf("bvs 0x%08X\n", rb.num_value - 3);
		    sprintf(core_current.Execute_instr_disp,
			    "Execute: bvs 0x%08X", rb.num_value - 3);
		    break;
		...
		case 0x7:	//b vc v clear
			...
			printf("bvc 0x%08X\n", rb.num_value - 3);
		    sprintf(core_current.Execute_instr_disp,
			    "Execute: bvc 0x%08X", rb.num_value - 3);
		    break;

		case 0x8:	//b hi c set z clear
		    if ((core_current.cpsr & CPSR_C)
			&& (!(core_current.cpsr & CPSR_Z)))
			{
			    rc.num_value = rb.num_value;
                //TODO: update rc.exp
			}
			...
			printf("bhi 0x%08X\n", rb.num_value - 3);
		    sprintf(core_current.Execute_instr_disp,
			    "Execute: bhi 0x%08X", rb.num_value - 3);
		    break;

		case 0x9:	//b ls c clear or z set
		    if ((core_current.cpsr & CPSR_Z)
			|| (!(core_current.cpsr & CPSR_C)))
			{
			    rc.num_value = rb.num_value;
        //TODO: update rc.exp
			}
			...
			printf("bls 0x%08X\n", rb.num_value - 3);
		    sprintf(core_current.Execute_instr_disp,
			    "Execute: bls 0x%08X", rb.num_value - 3);
		    break;

		case 0xA:	//b ge N == V
		    ra.num_value = 0; //TODO: update ra.exp
		    if ((core_current.cpsr & CPSR_N)
			&& (core_current.cpsr & CPSR_V))
			ra.num_value++;
		    if ((!(core_current.cpsr & CPSR_N))
			&& (!(core_current.cpsr & CPSR_V)))
			ra.num_value++;
      //TODO: update ra.exp;
			...
			printf("bge 0x%08X\n", rb.num_value - 3);
		    sprintf(core_current.Execute_instr_disp,
			    "Execute: bge 0x%08X", rb.num_value - 3);
		    break;
      ...
      //TODO: update ra.exp;
			...
			printf("blt 0x%08X\n", rb.num_value - 3);
		    sprintf(core_current.Execute_instr_disp,
			    "Execute: blt 0x%08X", rb.num_value - 3);
		    break;
      ...
      //TODO: update ra.exp;
			...
			printf("bgt 0x%08X\n", rb.num_value - 3);
		    sprintf(core_current.Execute_instr_disp,
			    "Execute: bgt 0x%08X", rb.num_value - 3);
		    break;
      ...
      //TODO: update ra.exp;
        ...
        //TODO: update rc.exp
			...
			printf("ble 0x%08X\n", rb.num_value - 3);
		    sprintf(core_current.Execute_instr_disp,
			    "Execute: ble 0x%08X", rb.num_value - 3);
		    break;
    ...
    if ((inst & 0xF800) == 0xE000)
	{
	    ...
	    rb.num_value <<= 1;
        //TODO: update rb.exp
	    rb.num_value += read_register2(15);

	    if (DEBUG_CORE)
		printf("B 0x%08X\n", rb.num_value - 3);
	    sprintf(core_current.Execute_instr_disp, "Execute: B 0x%08X",
		    rb.num_value - 3);
	    //Update ALU output
    ...
    if ((inst & 0xFFC0) == 0x4380)
	{
	    ...
	    if (DEBUG_CORE)
		printf("bics r%u,r%u\n", ra_ind, rb_ind);
	    sprintf(core_current.Execute_instr_disp, "Execute: bics r%u,r%u",
		    ra_ind, rb_ind);
	    //Functional
	    rc.num_value = ra.num_value & (~(rb.num_value));
        //TODO: update rc.exp
    ...
    if ((inst & 0xE000) == 0xE000)	//BL,BLX
	{
	    ...
	    if ((inst & 0x1800) == 0x1000)	//H=b10
		{
            ...
            rd_ind = 14;
		    if (rb.num_value & 1 << 10)
			rb.num_value |= (~((1 << 11) - 1));	//sign extend
		    rb.num_value <<= 12;
		    rb.num_value += read_register2(15) - 2;
		    if (DEBUG_CORE)
			printf("b1 pre-fix\n");
		    sprintf(core_current.Execute_instr_disp,
			    "Execute: b1 pre-fix");
		...
		{
		    //branch to thumb
		    read_register_forward(14, &rb);
		    rb.num_value += core_current.Execute_Imm;
		    rb.num_value += 2;
            //TODO: update rb.exp
		    rd_ind = 15;
		    write_register_value(14, (read_register2(15) - 4) | 1);
            //TODO: update reg[14].exp ?
		    if (DEBUG_CORE)
			printf("bl 0x%08X\n", rb.num_value - 3);
		    sprintf(core_current.Execute_instr_disp,
			    "Execute: bl 0x%08X", rb.num_value - 3);
		    //Update ALU output
		    update_component(&(core_current.Execute_ALU_result), &rb);
		    core_current.Execute_destination_regindex = 15;
		    write_register(core_current.Execute_destination_regindex,
	    ...
	    else if ((inst & 0x1800) == 0x0800)	//H=b01
		{
		    ...
		    read_register(14, &rb);
		    rb.num_value += core_current.Execute_Imm;
		    rb.num_value &= 0xFFFFFFFC;
		    rb.num_value += 2;
            //TODO: update rb.exp
		    rd_ind = 15;

		    write_register_value(14, (read_register2(15) - 2) | 1);
		    if (DEBUG_CORE)
			printf("bl 0x%08X\n", rb.num_value - 3);
	...
	{
	    ra_ind = core_current.Decode_port_regindex[0];
	    update_component(&rc, &(core_current.D2E_reg1));
	    rd_ind = 15;
	    rc.num_value += 2;
	    core_current.cpsr_valid = false;
	    if (rc.num_value & 1)
		{
		    ...
		    if (DEBUG_CORE)
			printf("blx r%u\n", ra_ind);
		    sprintf(core_current.Execute_instr_disp, "Execute:blx r%u",
			    ra_ind);
		    //Update ALU output
		    update_component(&(core_current.Execute_ALU_result), &rc);
		    core_current.Execute_destination_regindex = 15;
    ...
    if ((inst & 0xFFC0) == 0x42C0)
	{
	    ...
	    if (DEBUG_CORE)
		printf("cmns r%u,r%u\n", ra_ind, rb_ind);
	    sprintf(core_current.Execute_instr_disp, "Execute:cmns r%u,r%u",
		    ra_ind, rb_ind);
    ...
    if ((inst & 0xF800) == 0x2800)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp, "Execute:cmp r%u,#0x%02X",
		    ra_ind, rb.num_value);
	    //Functional
	    rc.num_value = ra.num_value - rb.num_value;
        //TODO: update rc.exp
    ...
    if ((inst & 0xFFC0) == 0x4280)
	{
	    ...
	    if (DEBUG_CORE)
		printf("cmps r%u,r%u\n", ra_ind, rb_ind);
	    sprintf(core_current.Execute_instr_disp, "Execute:cmps r%u,r%u",
		    ra_ind, rb_ind);
	    //Functional
	    rc.num_value = ra.num_value - rb.num_value;
        //TODO: update rc.exp
    ...
    if ((inst & 0xF800) == 0xC800)
	{
	    ...
	    while (rb_ind < 8)
		{
		    rb.num_value = (0x1 << rb_ind) & 0xff;
            //TODO: Update rb.exp
		    if (inst & rb.num_value)	//Find one register to fill
			{
			    core_current.Memory_read_targetreg = rb_ind;	//sent to memory
			    update_component(&(core_current.Memory_addr), &ra);
			    core_current.Read_valid = true;
			    core_current.Read_type = 0;
			    core_current.Execute_multicycle_regindex = rb_ind + 1;	//next register
			    sprintf(core_current.Execute_instr_disp,
				    "Execute: STMIA r%u=0x%X", rb_ind, ra.num_value);
			    ra.num_value = ra.num_value + 4;
                //TODO: update ra.exp
                update_component(&(core_current.D2E_reg1_data), &ra);	//update address
		...
		{
		    //Update ALU output
		    update_component(&(core_current.Execute_ALU_result), &ra);
    ...
    if ((inst & 0xF800) == 0x6800)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp,
		    "Execute: ldr r%u,[r%u,#0x%X]", rd_ind, ra_ind, rb.num_value);
	    core_current.Memory_read_targetreg = rd_ind;	//sent to memory
	    update_component(&(core_current.Memory_addr), &rc);;
	    core_current.Read_valid = true;
	    core_current.Read_type = 0;
	    //Update ALU output
	    update_component(&(core_current.Execute_ALU_result), &rc);
    ...
    if ((inst & 0xFE00) == 0x5800)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp,
		    "Execute: ldr r%u,[r%u,r%u]", rd_ind, ra_ind, rb_ind);
	    core_current.Memory_read_targetreg = rd_ind;	//sent to memory
	    update_component(&(core_current.Memory_addr), &rc);;
	    core_current.Read_valid = true;
	    core_current.Read_type = 0;
    ...
    if ((inst & 0xF800) == 0x4800)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp,
		    "Execute: ldr r%u,[PC+#0x%X]", rd_ind, rb.num_value);
	    core_current.Memory_read_targetreg = rd_ind;	//sent to memory
	    update_component(&(core_current.Memory_addr), &rc);;
	    core_current.Read_valid = true;
	    core_current.Read_type = 0;
    ...
    if ((inst & 0xF800) == 0x9800)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp,
		    "Execute: ldr r%u,[SP+#0x%X]", rd_ind, rb.num_value);
	    core_current.Memory_read_targetreg = rd_ind;	//sent to memory
	    update_component(&(core_current.Memory_addr), &rc);;
	    core_current.Read_valid = true;
	    core_current.Read_type = 0;
    ...
    if ((inst & 0xF800) == 0x7800)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp,
		    "Execute: ldrb r%u,[r%u,#0x%X]", rd_ind, ra_ind, rb.num_value);
	    core_current.Memory_read_targetreg = rd_ind;	//sent to memory
	    update_component(&(core_current.Memory_addr), &rc);;
	    core_current.Read_valid = true;
    ...
    if ((inst & 0xFE00) == 0x5C00)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp,
		    "Execute: ldrb r%u,[r%u,r%u]", rd_ind, ra_ind, rb_ind);
	    core_current.Memory_read_targetreg = rd_ind;	//sent to memory
	    update_component(&(core_current.Memory_addr), &rc);;
	    core_current.Read_valid = true;
	    core_current.Read_type = 1;
    ...
    if ((inst & 0xF800) == 0x8800)
	{
	    ...
	    if (check_delay(ra_ind) == true)
		{
		    ...
		    sprintf(core_current.Execute_instr_disp,
			    "Execute: stall by LDR");
		    //nullified the target index propagated from the last Decode
		    core_current.Execute_destination_regindex = 0xff;
		    //Re-do decode for LDR
		    Decode_OneCycle(true);
		    update_component(&(core_current.D2E_reg1), &(core_current.D2E_reg1_data));;
	    ...
	    sprintf(core_current.Execute_instr_disp,
		    "Execute: ldrh r%u,[r%u,#0x%X]", rd_ind, ra_ind, rb.num_value);
    ...
    if ((inst & 0xFE00) == 0x5A00)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp,
		    "Execute: ldrh r%u,[r%u,r%u]", rd_ind, ra_ind, rb_ind);
	    core_current.Memory_read_targetreg = rd_ind;	//sent to memory
	    update_component(&(core_current.Memory_addr), &rc);;
	    core_current.Read_valid = true;
	    core_current.Read_type = 2;
    ...
    if ((inst & 0xFE00) == 0x5600)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp,
		    "Execute: ldrsb r%u,[r%u,r%u]", rd_ind, ra_ind, rb_ind);
	    core_current.Memory_read_targetreg = rd_ind;	//sent to memory
	    update_component(&(core_current.Memory_addr), &rc);;
    ...
    if ((inst & 0xFE00) == 0x5E00)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp,
		    "Execute: ldrsh r%u,[r%u,r%u]", rd_ind, ra_ind, rb_ind);
	    core_current.Memory_read_targetreg = rd_ind;	//sent to memory
	    update_component(&(core_current.Memory_addr), &rc);;
    ...
    if ((inst & 0xF800) == 0x0000)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp,
		    "Execute: lsls r%u,r%u,#0x%X", rd_ind, ra_ind, rb.num_value);
	    //Functional
	    rc.num_value = ra.num_value;
        //TODO: update rc.exp
	    if (rb.num_value != 0)
		{
		    ...
		    shifted = shifted << (32 - rb.num_value);
		    do_cflag_bit(rc.num_value & shifted);
		    rc.num_value <<= rb.num_value;
		}
	    //Update CPSR
	    do_nflag(rc.num_value);
    ...
    if ((inst & 0xF800) == 0x0800)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp,
		    "Execute: lsrs r%u,r%u,#0x%X", rd_ind, ra_ind, rb.num_value);
	    rc.num_value = ra.num_value;
        //TODO: update rc.exp
	    //Functional
	    if (rb.num_value == 0)
		{
		    do_cflag_bit(rc.num_value & 0x80000000);
		    rc.num_value = 0;
            //TODO: update rc.exp
		}
	    else
		{
		    do_cflag_bit(rc.num_value & (1 << (rb.num_value - 1)));
		    rc.num_value >>= rb.num_value;
            //TODO: update rc.exp
		}
	    //Update CPSR
    ...
    if ((inst & 0xFFC0) == 0x40C0)
	{
	    ...
	    if (DEBUG_CORE)
		printf("lsrs r%u,r%u\n", ra_ind, rb_ind);
	    sprintf(core_current.Execute_instr_disp, "Execute: lsrs r%u,r%u",
		    ra_ind, rb_ind);
	    //Functional
	    rc.num_value = ra.num_value;
        //TODO: update rc.exp
    ...
    if ((inst & 0xF800) == 0x2000)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp,
		    "Execute: movs r%u,#0x%02X", rd_ind, rb.num_value);
	    //Functional
	    rc.num_value = rb.num_value;
        //TODO: update rc.exp
    ...
    if ((inst & 0xFFC0) == 0x1C00)
	{
	    ...
	    if (DEBUG_CORE)
		printf("movs r%u,r%u\n", rd_ind, ra_ind);
	    sprintf(core_current.Execute_instr_disp, "Execute: movs r%u,r%u",
		    rd_ind, ra_ind);
	    //Functional
	    rc.num_value = ra.num_value;
        //TODO: update rc.exp
    ...
    if ((inst & 0xFF00) == 0x4600)
	{
	    ...
	    if (DEBUG_CORE)
		printf("mov r%u,r%u\n", rd_ind, rb_ind);
	    sprintf(core_current.Execute_instr_disp, "Execute: mov r%u,r%u",
		    rd_ind, rb_ind);
	    //Functional
	    rc.num_value = rb.num_value;
        //TODO: update rc.exp
	    if (rd_ind == 15)
		{
		    rc.num_value &= ~1;	//write_register may do this as well
		    rc.num_value += 2;	//The program counter is special
            //TODO: update rc.exp
		}
	    //Update CPSR
	    //do_nflag(rc.num_value);
    ...
    if ((inst & 0xFFC0) == 0x4340)
	{
	    ...
	    if (DEBUG_CORE)
		printf("muls r%u,r%u\n", ra_ind, rb_ind);
	    sprintf(core_current.Execute_instr_disp, "Execute: muls r%u,r%u",
		    ra_ind, rb_ind);
	    //Functional
	    rc.num_value = ra.num_value * rb.num_value;
        //TODO: update rc.exp
    ...
    if ((inst & 0xFFC0) == 0x43C0)
	{
	    ...
	    if (DEBUG_CORE)
		printf("mvns r%u,r%u\n", rd_ind, rb_ind);
	    sprintf(core_current.Execute_instr_disp, "Execute: mvns r%u,r%u",
		    rd_ind, rb_ind);
	    //Functional
	    rc.num_value = ~(rb.num_value);
        //TODO: update rc.exp
    ...
    if ((inst & 0xFFC0) == 0x4240)
	{
	    ...
	    //do_cflag_bit(0);
	    //do_vflag_bit(0);
	    do_cflag(0, ~(rb.num_value), 1);
	    do_vflag(0, ~(rb.num_value), 1);
	    core_current.cpsr_valid = true;
	    //Update ALU output
	    update_component(&(core_current.Execute_ALU_result), &rc);
	    return false;
    ...
    if ((inst & 0xFFC0) == 0x4300)
	{
	    ...
	    if (DEBUG_CORE)
		printf("orrs r%u,r%u\n", ra_ind, rb_ind);
	    sprintf(core_current.Execute_instr_disp, "Execute: orrs r%u,r%u",
		    ra_ind, rb_ind);
	    //Functional
	    rc.num_value = ra.num_value | rb.num_value;
        //TODO: update rc.exp
    ...
    if ((inst & 0xFE00) == 0xB400)
	{
	    ...
	    if (rb_ind == 9)
		{
		    ...
		    if (inst & 0x100)	//push lr
			{
			    ra.num_value = ra.num_value - 4;
                //TODO: update ra.exp
			    read_register(14, &(core_current.Memory_data));	//sent to memory
			    update_component(&(core_current.Memory_addr), &ra);
			    sprintf(core_current.Execute_instr_disp,
				    "Execute: push lr=0x%X",
	    ...
	    while (rb_ind < 8)
		{
		    rb.num_value = (0x1 << (7 - rb_ind)) & 0xff;
            //TODO: update rb.exp
		    if (inst & rb.num_value)	//Find one register to fill
			{
			    ra.num_value = ra.num_value - 4;
                //TODO: update ra.exp
			    read_register(7 - rb_ind, &(core_current.Memory_data));	//sent to memory
			    sprintf(core_current.Execute_instr_disp,
				    "Execute: push r%u=0x%X", 7 - rb_ind,
				    core_current.Memory_data.num_value);
    ...
    if ((inst & 0xFFC0) == 0xBA00)
	{
	    ...
	    if (DEBUG_CORE)
		printf("rev r%u,r%u\n", rd_ind, rb_ind);
	    sprintf(core_current.Execute_instr_disp, "Execute: rev r%u,r%u",
		    rd_ind, rb_ind);

	    //Functional
	    rc.num_value = ((rb.num_value >> 0) & 0xFF) << 24;
	    rc.num_value |= ((rb.num_value >> 8) & 0xFF) << 16;
	    rc.num_value |= ((rb.num_value >> 16) & 0xFF) << 8;
	    rc.num_value |= ((rb.num_value >> 24) & 0xFF) << 0;
        //TODO: update rc.exp
	    //Update CPSR
	    //do_nflag(rc.num_value);
	    //do_zflag(rc.num_value);
    ...
    if ((inst & 0xFFC0) == 0xBA40)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp, "Execute: rev16 r%u,r%u",
		    rd_ind, rb_ind);
	    //Functional
	    rc.num_value = ((rb.num_value >> 0) & 0xFF) << 8;
	    rc.num_value |= ((rb.num_value >> 8) & 0xFF) << 0;
	    rc.num_value |= ((rb.num_value >> 16) & 0xFF) << 24;
	    rc.num_value |= ((rb.num_value >> 24) & 0xFF) << 16;
        //TODO: update rc.exp
	    //Update CPSR
	    //do_nflag(rc.num_value);
	    //do_zflag(rc.num_value);
    ...
    if ((inst & 0xFFC0) == 0xBAC0)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp, "Execute: revsh r%u,r%u",
		    rd_ind, rb_ind);
	    //Functional
	    rc.num_value = ((rb.num_value >> 0) & 0xFF) << 8;
	    rc.num_value |= ((rb.num_value >> 8) & 0xFF) << 0;
    ...
    if ((inst & 0xFFC0) == 0x41C0)
	{
	    ...
	    if (DEBUG_CORE)
		printf("rors r%u,r%u\n", rd_ind, rb_ind);
	    sprintf(core_current.Execute_instr_disp, "Execute: rors r%u,r%u",
		    rd_ind, rb_ind);
	    //Functional
	    rb.num_value &= 0xFF;
        //TODO: update rb.exp
	    ...
	    else
		{
			...
			{
			    do_cflag_bit(rc.num_value & (1 << (rb.num_value - 1)));
			    ra.num_value = rc.num_value << (32 - rb.num_value);
			    rc.num_value >>= rb.num_value;
			    rc.num_value |= ra.num_value;
                //TODO: update rc.exp
			}
    ...
    if ((inst & 0xFFC0) == 0x4180)
	{
	    ...
	    if (DEBUG_CORE)
		printf("sbc r%u,r%u\n", ra_ind, rb_ind);
	    sprintf(core_current.Execute_instr_disp, "Execute: sbc r%u,r%u",
		    ra_ind, rb_ind);
	    //Logical calculation 
	    rc.num_value = ra.num_value - rb.num_value;
        //TODO: update rc.exp
    ...
    if ((inst & 0xF800) == 0xC000)
	{
	    ...
	    while (rb_ind < 8)
		{
		    ...
		    if (inst & rb.num_value)	//Find one register to fill
			{
			    read_register(rb_ind, &(core_current.Memory_data));	//sent to memory
			    update_component(&(core_current.Memory_addr), &ra);
			    sprintf(core_current.Execute_instr_disp,
				    "Execute: stmia r%u", rb_ind);
			    core_current.Write_valid = true;
			    core_current.Write_type = 0;
			    core_current.Execute_multicycle_regindex = rb_ind + 1;	//next register
			    ra.num_value = ra.num_value + 4;
	    ...
	    else
		{
			...
			{
			    read_register(15, &(core_current.Memory_data));	//sent to memory
			    update_component(&(core_current.Memory_addr), &ra);
			    sprintf(core_current.Execute_instr_disp,
				    "Execute: stmia pc");
			    core_current.Write_valid = true;
    ...
    if ((inst & 0xF800) == 0x6000)
	{
	    ...
	    update_component(&ra, &(core_current.D2E_reg1));
	    update_component(&rb, &(core_current.D2E_reg2));
	    rc.num_value = core_current.Execute_Imm;
        //TODO: update rc.exp
	    ra_ind = core_current.Decode_port_regindex[1];
	    rb_ind = core_current.Decode_port_regindex[0];
	    rd_ind = 0xff;	//not used
	    if (DEBUG_CORE)
		printf("str r%u,[r%u,#0x%X]\n", rb_ind, ra_ind, rc.num_value);
	    sprintf(core_current.Execute_instr_disp,
		    "Execute: str r%u,[r%u,#0x%X]", rb_ind, ra_ind, rc.num_value);
	    //Functional
	    rc.num_value <<= 2;
        //TODO: update rc.exp
	    rc.num_value = ra.num_value + rc.num_value;
        //TODO: update rc.exp
	    update_component(&(core_current.Memory_data), &rb);	//sent to memory
	    update_component(&(core_current.Memory_addr), &rc);;
	    core_current.Write_valid = true;
	    core_current.Write_type = 0;
	    core_current.cpsr_valid = false;
    ...
    if ((inst & 0xFE00) == 0x5000)
	{
	    if (core_current.Execute_multicycle_regindex == 9)	//First cycle
		{
			...
			printf("Execute C1:str r%u,[r%u,r%u]\n", rd_ind, ra_ind,
			       rb_ind);
		    sprintf(core_current.Execute_instr_disp,
			    "Execute C1: str r%u,[r%u,r%u]", rd_ind, ra_ind,
			    rb_ind);
		    ...
		    //Update ALU output
		    update_component(&(core_current.Execute_ALU_result), &rc);
		    read_register_forward(rd_ind, &(core_current.D2E_reg1_data));
		    core_current.D2E_reg1_valid = true;
		    core_current.D2E_reg2_valid = false;
		    core_current.Execute_destination_regindex = 0xff;
		    core_current.cpsr_valid = false;
	    ...
	    else		//Second cycle
		{
			...
			printf("Execute C2:str r%u,[r%u,r%u]\n", ra_ind, rd_ind,
			       rb_ind);
		    sprintf(core_current.Execute_instr_disp,
			    "Execute C2: str r%u,[r%u,r%u]", ra_ind, rd_ind,
			    rb_ind);
		    rd_ind = 0xff;	//not used
		    core_current.Execute_multicycle_regindex = 9;
		    update_component(&(core_current.Memory_data), &ra);	//sent to memory
		    core_current.Write_valid = true;
		    core_current.Write_type = 0;
		    //Update ALU output
		    update_component(&(core_current.Execute_ALU_result), &ra);
    ...
    if ((inst & 0xF800) == 0x9000)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp,
		    "Execute: str r%u,[SP,#0x%X]", rb_ind, rc.num_value);
	    //Functional
	    rc.num_value <<= 2;
        //TODO: update rc.exp/
	    rc.num_value = ra.num_value + rc.num_value;
    ...
    if ((inst & 0xF800) == 0x7000)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp,
		    "Execute: strb r%u,[r%u,#0x%X]", rb_ind, ra_ind, rc.num_value);
	    //Functional
	    rc.num_value = ra.num_value + rc.num_value;
        //TODO: update rc.exp
	    update_component(&(core_current.Memory_data), &rb);	//sent to memory
    ...
    if ((inst & 0xFE00) == 0x5400)
	{
	    if (core_current.Execute_multicycle_regindex == 9)	//First cycle
		{
			...
			printf("C1:strb r%u,[r%u,r%u]\n", rd_ind, ra_ind,
			       rb_ind);
		    sprintf(core_current.Execute_instr_disp,
			    "Execute C1:strb r%u,[r%u,r%u]", rd_ind, ra_ind,
			    rb_ind);
	    ...
	    else		//Second cycle
		{
			...
			printf("C2:strb r%u,[r%u,r%u]\n", ra_ind, rd_ind,
			       rb_ind);
		    sprintf(core_current.Execute_instr_disp,
			    "Execute C2:strb r%u,[r%u,r%u]", ra_ind, rd_ind,
			    rb_ind);
		    rd_ind = 0xff;	//not used
		    core_current.Execute_multicycle_regindex = 9;
		    update_component(&(core_current.Memory_data), &ra);	//sent to memory
    ...
    if ((inst & 0xF800) == 0x8000)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp,
		    "Execute: strh r%u,[r%u,#0x%X]", rb_ind, ra_ind, rc.num_value);
    ...
    if ((inst & 0xFE00) == 0x5200)
	{
	    if (core_current.Execute_multicycle_regindex == 9)	//First cycle
		{
			...
			printf("C1:strh r%u,[r%u,r%u]\n", rd_ind, ra_ind,
			       rb_ind);
		    sprintf(core_current.Execute_instr_disp,
			    "Execute C1:strh r%u,[r%u,r%u]", rd_ind, ra_ind,
			    rb_ind);
		    core_current.Execute_multicycle_regindex = 0;
		    //Functional
		    rc.num_value = ra.num_value + rb.num_value;
	    ...
	    else		//Second cycle
		{
			...
			printf("C2:strh r%u,[r%u,r%u]\n", ra_ind, rd_ind,
			       rb_ind);
		    sprintf(core_current.Execute_instr_disp,
			    "Execute C2:strh r%u,[r%u,r%u]", ra_ind, rd_ind,
			    rb_ind);
    ...
    if ((inst & 0xFE00) == 0x1E00)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp,
		    "Execute:subs r%u,r%u,#0x%X", rd_ind, ra_ind, rb.num_value);
    ...
    if ((inst & 0xF800) == 0x3800)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp, "Execute:subs r%u,#0x%02X",
		    ra_ind, rb.num_value);
	    //Logical calculation 
	    rc.num_value = ra.num_value - rb.num_value;
        //TODO: update rc.exp
    ...
    if ((inst & 0xFE00) == 0x1A00)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp,
		    "Execute: subs r%u,r%u,r%u", rd_ind, ra_ind, rb_ind);
	    //Functional
	    rc.num_value = ra.num_value - rb.num_value;
        //TODO: update rc.exp
    ...
    if ((inst & 0xFF80) == 0xB080)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp,
		    "Execute: sub r%u,SP,#0x%02X", rd_ind, rb.num_value);
	    //Functional
	    rc.num_value = ra.num_value - rb.num_value;
        //TODO: update rc.exp
    ...
    if ((inst & 0xFFC0) == 0xB240)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp, "Execute: sxtb r%u,r%u\n",
		    rd_ind, rb_ind);
	    //Functional
	    rc.num_value = rb.num_value & 0xFF;
        //TODO: update rc.exp
	    if (rc.num_value & 0x80)
		{
		    unsigned int tmp = ~0;
		    tmp <<= 8;
		    rc.num_value |= tmp;
    ...
    if ((inst & 0xFFC0) == 0xB200)
	{
	    ...
	    if (DEBUG_CORE)
		printf("sxth r%u,r%u\n", rd_ind, rb_ind);
	    sprintf(core_current.Execute_instr_disp, "Execute: sxth r%u,r%u",
		    rd_ind, rb_ind);
	    //Functional
	    rc.num_value = rb.num_value & 0xFFFF;
        //TODO: update rc.exp
	    if (rc.num_value & 0x8000)
		{
		    unsigned int tmp = ~0;
		    tmp <<= 16;
		    rc.num_value |= tmp;
    ...
    if ((inst & 0xFFC0) == 0x4200)
	{
	    ...
	    if (DEBUG_CORE)
		printf("tst r%u,r%u\n", ra_ind, rb_ind);
	    sprintf(core_current.Execute_instr_disp, "Execute: tst r%u,r%u",
		    ra_ind, rb_ind);
	    //Functional
	    rc.num_value = ra.num_value & rb.num_value;
        //TODO: update rc.exp
    ...
    if ((inst & 0xFFC0) == 0xB2C0)
	{
	    ...
	    if (DEBUG_CORE)
		printf("uxtb r%u,r%u\n", rd_ind, rb_ind);
	    sprintf(core_current.Execute_instr_disp, "Execute: uxtb r%u,r%u",
		    rd_ind, rb_ind);
	    //Functional
	    rc.num_value = rb.num_value & 0xFF;
        //TODO: update rc.exp
