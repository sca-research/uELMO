...
bool Execute_OneCylce(bool wait_mem)
{
    ...
    if ((inst & 0xF800) == 0x3000)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp,
		    "Execute: adds r%u,#0x%02X", rd_ind, rb);
    ...
    if ((inst & 0xF800) == 0xA000)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp,
		    "Execute: add r%u,PC,#0x%02X", rd_ind, rb);
	    //Functional
	    rc = ra + rb;
    ...
    if ((inst & 0xF800) == 0xA800)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp,
		    "Execute: add r%u,SP,#0x%02X", rd_ind, rb);
    ...
    if ((inst & 0xF800) == 0x1000)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp,
		    "Execute: asrs r%u,r%u,#0x%X", rd_ind, ra_ind, rb);
    ...
    if ((inst & 0xF800) == 0xC800)
	{
	    if (check_delay(core_current.Decode_port_regindex[2]) == true)
		{
		    ...
		    core_current.D2E_reg1 = core_current.D2E_reg1_data;
		    core_current.D2E_reg2 = core_current.D2E_reg2_data;
		    SymCopy(sym_core_current.D2E_reg1,
			    sym_core_current.D2E_reg1_data);
		    SymCopy(sym_core_current.D2E_reg2,
			    sym_core_current.D2E_reg2_data);
		    return true;
		}
	    //According to paper
	    ra = core_current.D2E_reg1;
	    ...
	    while (rb_ind < 8)
		{
		    ...
		    if (inst & rb)	//Find one register to fill
			{
			    ...
			    sprintf(core_current.Execute_instr_disp,
				    "Execute: LDMIA r%u=0x%X", rb_ind, ra);
			    ra = ra + 4;
			    ...
			    SymAssign(sym_core_current.Memory_addr,
				      SymGetMemAddrAnnotation());
			    SymAssign(sym_core_current.D2E_reg1_data,
				      SymGetMemAddrAnnotation());
    ...
    if ((inst & 0xF800) == 0x6800)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp,
		    "Execute: ldr r%u,[r%u,#0x%X]", rd_ind, ra_ind, rb);
    ...
    if ((inst & 0xF800) == 0x4800)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp,
		    "Execute: ldr r%u,[PC+#0x%X]", rd_ind, rb);
	    core_current.Memory_read_targetreg = rd_ind;	//sent to memory
	    core_current.Memory_addr = rc;
	    core_current.Read_valid = true;
	    core_current.Read_type = 0;
    ...
    if ((inst & 0xF800) == 0x9800)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp,
		    "Execute: ldr r%u,[SP+#0x%X]", rd_ind, rb);
	    core_current.Memory_read_targetreg = rd_ind;	//sent to memory
	    core_current.Memory_addr = rc;
	    core_current.Read_valid = true;
	    core_current.Read_type = 0;
    ...
    if ((inst & 0xF800) == 0x7800)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp,
		    "Execute: ldrb r%u,[r%u,#0x%X]", rd_ind, ra_ind, rb);
	    core_current.Memory_read_targetreg = rd_ind;	//sent to memory
	    core_current.Memory_addr = rc;
	    core_current.Read_valid = true;
    ...
    if ((inst & 0xF800) == 0x8800)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp,
		    "Execute: ldrh r%u,[r%u,#0x%X]", rd_ind, ra_ind, rb);
    ...
    if ((inst & 0xF800) == 0x0000)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp,
		    "Execute: lsls r%u,r%u,#0x%X", rd_ind, ra_ind, rb);
	    //Functional
	    rc = ra;
	    if (rb != 0)
		{
    ...
    if ((inst & 0xF800) == 0x0800)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp,
		    "Execute: lsrs r%u,r%u,#0x%X", rd_ind, ra_ind, rb);
	    rc = ra;
	    //Functional
	    if (rb == 0)
		{
    ...
    if ((inst & 0xF800) == 0x2000)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp,
		    "Execute: movs r%u,#0x%02X", rd_ind, rb);
	    //Functional
	    rc = rb;
    ...
    if ((inst & 0xF800) == 0x6000)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp,
		    "Execute: str r%u,[r%u,#0x%X]", rb_ind, ra_ind, rc);
	    //Functional
	    rc <<= 2;
	    rc = ra + rc;
    ...
    if ((inst & 0xF800) == 0x9000)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp,
		    "Execute: str r%u,[SP,#0x%X]", rb_ind, rc);
	    //Functional
	    rc <<= 2;
	    rc = ra + rc;
	    core_current.Memory_data = rb;	//sent to memory
    ...
    if ((inst & 0xF800) == 0x7000)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp,
		    "Execute: strb r%u,[r%u,#0x%X]", rb_ind, ra_ind, rc);
	    //Functional
	    rc = ra + rc;
	    core_current.Memory_data = rb;	//sent to memory
	    core_current.Memory_addr = rc;
    ...
    if ((inst & 0xF800) == 0x8000)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp,
		    "Execute: strh r%u,[r%u,#0x%X]", rb_ind, ra_ind, rc);
    ...
    if ((inst & 0xFE00) == 0x1E00)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp,
		    "Execute:subs r%u,r%u,#0x%X", rd_ind, ra_ind, rb);
    ...
    if ((inst & 0xFF80) == 0xB080)
	{
	    ...
	    sprintf(core_current.Execute_instr_disp,
		    "Execute: sub r%u,SP,#0x%02X", rd_ind, rb);
	    //Functional
	    rc = ra - rb;
