...
bool Memory_OneCycle()
{
    ...
    if (core_current.Read_valid)	//Performing read
	{
	    ...
	    sprintf(core_current.Memory_instr_disp, "Memory: load [0x%X]=0x%X",
		    core_current.Memory_addr, value);
    ...
    if (core_current.Write_valid)	//Performing write
	{
		...
		case 1:	//byte
			...
			switch (core_current.Memory_addr & 0x1)
				...
				value = value & 0xff00;
				value =
				    value | (core_current.Memory_data & 0xff);
				write16(core_current.Memory_addr & 0xfffffffe,
					value);
				break;
			    case 1:
				value =
	    ...
	    sprintf(core_current.Memory_instr_disp, "Memory: store [0x%X]=0x%X",
		    core_current.Memory_addr, core_current.Memory_data);
